/**
 * SSOLoginButton.tsx
 * Single Sign-On login button component
 *
 * Initiates SSO authentication flow for Google, Microsoft, or SAML providers
 */

import React, { useState } from 'react';
import { LogIn, Loader2 } from 'lucide-react';
import { cn } from '../../../utils/helpers';

export type SSOProvider = 'google' | 'microsoft' | 'saml';

export interface SSOLoginButtonProps {
  provider: SSOProvider;
  displayName: string;
  icon?: React.ReactNode;
  tenantId?: string;
  onError?: (error: Error) => void;
  className?: string;
  disabled?: boolean;
}

export const SSOLoginButton: React.FC<SSOLoginButtonProps> = ({
  provider,
  displayName,
  icon,
  tenantId,
  onError,
  className,
  disabled = false
}) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleLogin = async () => {
    try {
      setLoading(true);
      setError(null);

      // Initiate SSO flow
      const response = await fetch(`/api/v1/auth/sso/${provider}/initiate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          tenantId: tenantId || localStorage.getItem('tenantId'),
          redirectUrl: `${window.location.origin}/auth/sso/callback`
        })
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || `Failed to initiate ${displayName} login`);
      }

      const data = await response.json();

      if (!data.success || !data.authUrl) {
        throw new Error('Invalid response from SSO server');
      }

      // Store state token for CSRF validation
      if (data.state) {
        sessionStorage.setItem('sso_state', data.state);
        sessionStorage.setItem('sso_provider', provider);
      }

      // Redirect to SSO provider
      window.location.href = data.authUrl;

    } catch (err: any) {
      const errorMessage = err.message || 'Failed to initiate SSO login';
      setError(errorMessage);

      if (onError) {
        onError(new Error(errorMessage));
      }

      console.error(`[SSO] ${provider} login error:`, err);
      setLoading(false);
    }
  };

  const getProviderIcon = () => {
    if (icon) return icon;

    // Default provider icons
    switch (provider) {
      case 'google':
        return (
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
        );
      case 'microsoft':
        return (
          <svg className="w-5 h-5" viewBox="0 0 23 23">
            <path fill="#f3f3f3" d="M0 0h23v23H0z"/>
            <path fill="#f35325" d="M1 1h10v10H1z"/>
            <path fill="#81bc06" d="M12 1h10v10H12z"/>
            <path fill="#05a6f0" d="M1 12h10v10H1z"/>
            <path fill="#ffba08" d="M12 12h10v10H12z"/>
          </svg>
        );
      default:
        return <LogIn className="w-5 h-5" />;
    }
  };

  return (
    <div className="w-full">
      <button
        onClick={handleLogin}
        disabled={loading || disabled}
        className={cn(
          "flex items-center justify-center gap-3 w-full px-4 py-3",
          "bg-white border border-gray-300 rounded-lg shadow-sm",
          "hover:bg-gray-50 hover:border-gray-400",
          "focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500",
          "transition-all duration-200",
          "disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white",
          className
        )}
        aria-label={`Sign in with ${displayName}`}
      >
        {loading ? (
          <Loader2 className="w-5 h-5 animate-spin text-gray-600" />
        ) : (
          <span className="flex-shrink-0">{getProviderIcon()}</span>
        )}
        <span className="text-sm font-medium text-gray-700">
          {loading ? 'Redirecting...' : `Continue with ${displayName}`}
        </span>
      </button>

      {error && (
        <div className="mt-2 text-sm text-red-600" role="alert">
          {error}
        </div>
      )}
    </div>
  );
};

export default SSOLoginButton;
