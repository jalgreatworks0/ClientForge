...
export const authSchemas = {
  register: z.object({
    tenantId: commonSchemas.tenantId.optional().default(DEFAULT_TENANT_ID),
...
export async function login(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    // Parse and add default tenant ID if not provided
    ...
    const result = await authService.login({
      email: data.email,
      password: data.password,
      tenantId: tenantId, // Always pass tenant ID
      ipAddress,
      userAgent,
      deviceType,
    })
    ...
    console.log('[AUTH-CONTROLLER] Login successful for:', data.email)
