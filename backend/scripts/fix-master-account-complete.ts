...
async function fixMasterAccountComplete() {
  ...
  try {
    ...
    console.log('ðŸ”§ CLIENTFORGE CRM - COMPLETE AUTH FIX')
    console.log('='.repeat(60))
    ...
    await pool.query(
      `INSERT INTO tenants (id, name, slug, status, created_at, updated_at)
       ...
       SET name = EXCLUDED.name, 
           ...
           updated_at = NOW()`,
      [DEFAULT_tenantId, 'Default Organization', 'default', 'active']
    )
    console.log('âœ… Default tenant ready')
    ...
    if (existingUser.rows.length > 0) {
      ...
      console.log('âœ… Cleaned up existing master account')
    } else {
      console.log('âœ… No existing account to clean up')
    }
    ...
    console.log('\n' + '='.repeat(60))
    console.log('ðŸŽ‰ MASTER ACCOUNT SETUP COMPLETE!')
    console.log('='.repeat(60))
    console.log('\nðŸ“‹ ACCOUNT DETAILS:')
    console.log('â”€'.repeat(40))
    console.log('ðŸ“§ Email:        ', MASTER_EMAIL)
    console.log('ðŸ”‘ Password:     ', MASTER_PASSWORD)
    console.log('ðŸ¢ Tenant ID:    ', DEFAULT_tenantId)
    console.log('ðŸ‘¤ User ID:      ', userId)
    console.log('ðŸ›¡ï¸  Role:         ', user.role_name || 'admin')
    ...
    if (testQuery.rows.length > 0) {
      ...
      console.log('âœ… Password hash valid:', passwordValid)
      console.log('âœ… Account active:', testUser.is_active)
      console.log('âœ… Email verified:', testUser.email_verified)
      console.log('âœ… Not locked:', !testUser.locked_until)
      
      if (passwordValid && testUser.is_active) {
        console.log('\nðŸŽ‰ LOGIN TEST PASSED - Account is ready!')
      } else {
        console.log('\nâš ï¸  WARNING: Account exists but may have login issues')
      }
    } else {
      console.log('\nâŒ ERROR: User not found with tenant query')
    }

    console.log('\n' + '='.repeat(60))
    console.log('ðŸ“ CURL TEST COMMAND:')
    console.log('â”€'.repeat(40))
